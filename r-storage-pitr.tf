...
resource "azapi_update_resource" "sa_pitr" {
  for_each = toset(
    ...
    && try(var.storage_blob_data_protection.container_point_in_time_restore, false)
    && try(var.storage_blob_data_protection.container_delete_retention_policy_in_days > 2, false)
