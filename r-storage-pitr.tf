...
resource "azapi_update_resource" "sa_pitr" {
  for_each = toset(local.storage_blob_data_protection.container_point_in_time_restore && local.storage_blob_data_protection.container_delete_retention_policy_in_days > 2 && !var.nfsv3_enabled ? ["enabled"] : [])
